...
# directory word hier verwerkt. Deze verwijst naar de Path
ss.path += [os.path.abspath(relPath) for relPath in ('..',)]


# stuurt en ontvangt informatie van en naar/maakt een connectie tussen de auto en de wereld. Daarvoor wordt de socked_wrapper gebruikt
    ...
    def __init__(self):
        ...
        with open(pm.sampleFileName, 'w') as self.sampleFile:
            with sc.socket(*sw.socketType) as self.clientSocket:
                ...
                # programma blijft in de Loop tot deze wordt afgesloten
                while True:
                    self.input()
                    self.sweep()
                    self.output()
                    self.logTraining()
                    tm.sleep(0.02)  # deze berekend de aantal stuur momenten
# deze functie ontvangt de data

    def input(self):
        # ontvangt van de socket_wrapper de informatier die deze in de sensoren stopt
    ...
    def lidarSweep(self):
        # parameter staat op afstand 20, om niet te veel pylonen mee te laten reken
...
# hier wordt de data verwerkt

        for lidarAngle in range(-self.halfApertureAngle, self.halfApertureAngle):
            lidarDistance = self.lidarDistances[lidarAngle]
            # aanpassen dichtsbijzijnde pyon
            # de dichts bijzijnde pyon wordt vervangen door een andere, waardoor de dichtbijzijnde de ena dichtsbijzijnde word.
    ...
    def sonarSweep(self):
        ...
        self.steeringAngle = (
            obstacleAngles[leftIndex] + obstacleAngles[rightIndex]) / 2
        self.targetVelocity = pm.getTargetVelocity(self.steeringAngle)
...
# informatie die wordt teurg gezonden naar de auto
    def output(self):
        actuators = {
...
# verzamelt trainings data en schrijft deze weg naar een document
